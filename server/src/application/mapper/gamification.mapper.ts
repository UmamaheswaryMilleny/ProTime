...
import type { UserBadgeEntity, BadgeDefinitionEntity } from '../../domain/entities/badge.entity';
import type {
...
export class GamificationMapper {
    ...
    // ─── UserBadge + BadgeDefinition → UserBadgeResponseDTO ──────────────────
    // Denormalizes badge name/description/icon from definition into earned badge
    ...
    // ─── UserGamificationEntity → GamificationResponseDTO ────────────────────
    // isPremium flag drives isTitleLocked — free users see lock above Learner
    static toResponse(
        ...
        const nextLevelXp = LEVEL_XP_THRESHOLDS[nextLevel] ?? LEVEL_XP_THRESHOLDS[MAX_LEVEL];
        ...
        const isTitleLocked = !isPremium && !freeTitleOrder.includes(entity.currentTitle);
...
dailyChatMessageCount: entity.dailyChatMessageCount,
    ...
    // ─── AwardXp result → AwardXpResponseDTO ─────────────────────────────────
    ...
    // ─── New gamification entity → InitializeGamificationResponseDTO ──────────
    static toInitResponse(entity: UserGamificationEntity): InitializeGamificationResponseDTO {
        return {
            userId: entity.userId,
            totalXp: entity.totalXp,
            currentLevel: entity.currentLevel,
