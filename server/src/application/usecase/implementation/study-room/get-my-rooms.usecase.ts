...
import type { IGetMyRoomsUsecase } from "../../interface/study-room/get-my-rooms.usecase.interface";
...
export class GetMyRoomsUsecase implements IGetMyRoomsUsecase {
  ...
  async execute(hostId: string): Promise<StudyRoomResponseDTO[]> {
    const rooms = await this.studyRoomRepo.findByHostId(hostId);
    
    // Sort descending by creation date or filter logically if needed
