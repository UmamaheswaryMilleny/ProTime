...
export const userApi = {
  ...
  // ⚠️ Backend endpoint does not exist yet — will throw until route is built.
  // When ready: pass returned URL into updateProfileService({ profileImage: url })
  uploadProfileImage: async (_file: File): Promise<string> => {
    throw new Error(
      "uploadProfileImage: backend endpoint /user/upload/profile-image is not yet implemented."
    );
